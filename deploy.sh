...
s3cmd sync --progress -M --acl-public --add-header 'Content-Encoding:gzip' --add-header 'Cache-Control: max-age=86400' _site/static/ s3://css.dominicwatson.co.uk/frankfusion/ --exclude '*.*' --include '*.js' --include '*.css'
s3cmd sync --progress -M --acl-public --add-header 'Cache-Control: max-age=86400' _site/static/ s3://css.dominicwatson.co.uk/frankfusion/ --exclude '*.css' --exclude '*.js'
